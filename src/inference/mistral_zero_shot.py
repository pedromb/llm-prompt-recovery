...
Strategy:
    - Single Prompt
LB Score: 
    mistralai/Mistral-7B-Instruct-v0.2:  0.61/0.63 (with mean prompt)
val score: 
   mistralai/Mistral-7B-Instruct-v0.2:   0.648
val std:   
   mistralai/Mistral-7B-Instruct-v0.2:   0.097
"""
...
def batch_get_rewrite_prompts(model, tokenizer, original_text, rewritten_text):
    ...
    generated_texts = run_inference(model, tokenizer, prompts, do_sample=False)
    rewrite_prompts = []
    for idx, rw_prompt in enumerate(generated_texts):
        tries = 5
        while tries > 0:
            try:
                if tries != 5:
                    ...
                    rw_prompt = run_inference(model, tokenizer, prompt, do_sample=True)[0]
